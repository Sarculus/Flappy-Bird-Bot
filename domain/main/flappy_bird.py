...
class FlappyBird:
    ...
    def start_gameplay_loop(self, frame_count, top_left_x_y_cor, pipe_position):
        while True:
            ...
            frame_count += 1
            bird_position = self.get_bird_position() + 20  # bird head to centre is 20 px
            pipe_position_update = self.get_pipe_position()  # + 90  #half of pipe gape is 90 px
            if pipe_position_update != 0:
                pipe_position = pipe_position_update
            print(bird_position, pipe_position)
            if pipe_position == -1 or bird_position == -1:
                self.go_down()  # self.go_steady()
            elif bird_position < pipe_position:
                self.go_down()
            elif bird_position > pipe_position + 180:  # 180 is pipe gap in pixels
                self.go_up()
            else:
                self.go_steady()
            print("--------------------------")
    ...
    def update_saved_screen(self, count, game_cor):
        ...
        with mss.mss() as sct:
            # sct.shot(output=img_name)  # taking a screenshot and saving it to an image file
